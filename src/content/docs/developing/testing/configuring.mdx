...
![](/src/assets/i_testing/Testing_TestingJson.png)
...
![](/src/assets/i_testing/Testing_ExecutionConfiguration.png)
...
<Tabs>
  <TabItem label="RUCRTRPG" >
    ...
    |`text`       |Text description               |Allows you to enter text that briefly describes the RPGUnit test case and its function.                                                                                                                                     |*SRCMBRTXT |
    ...
    |`define`     |Define condition names         |Specifies condition names that are defined before the compilation begins. Using the parameter `DEFINE(condition-name)` is equivalent to coding the `/DEFINE` condition-name directive on the first line of the source file. |*NONE      |
    |`dltSplf`    |Delete spooled files           |Specifies if all spooled files (compile listing) created by the object creations commands are deleted on success.                                                                                                           |*NO        |
    |`dltMod`     |Delete modules                 |Specifies if all modules created by the object creations commands are deleted on success.                                                                                                                                   |*YES       |
    |`actGrp`     |Activation group               |Refer to the `ACTGRP` parameter in `CRTSRVPGM` command help.                                                                                                                                                                |*CALLER    |
    ...
    |`pOption`    |Pre-compiler OPTIONS           |Refer to the `OPTION` parameter in `CRTSQLRPGI` command help.                                                                                                                                                               |*SYSVAL    |
    |`compileOpt` |Pre-Compiler COMPILEOPT        |Refer to the `COMPILEOPT` parameter in `CRTSQLRPGI` command help.                                                                                                                                                           |           |
    |`cmpparm`    |Additional compiler parameters |Specifies any additional compiler parameters for producing the command for compiling the module.                                                                                                                            |           |
    |`lnkparm`    |Additional linker parameters   |Specifies any additional linker parameters for producing the command for linking the unit test service program.                                                                                                             |           |
    |`tgtRls`     |Target release                 |Refer to the `TGTRLS` parameter in `CRTSRVPGM` command help.                                                                                                                                                                |*CURRENT   |
    |`incDir`     |Include directory              |Specifies one or more directories to add to the search path used by the compiler to find copy files. The compiler will search the directories specified here if the copy files in the source program can not be resolved.   |*NONE      |
    |`tgtCcsid`   |Target CCSID                   |Specifies the CCSID that the compiler uses to read the source files.                                                                                                                                                        |*JOB       |
    |`proxyCmd`   |Proxy Command                  |Specifies a command to substitute the `RUCRTRPG` command.                                                                                                                                                                   |           |
  ...
  <TabItem label="RUCRTCBL" >
    ...
    |`actGrp`     |Activation group               |Refer to the `ACTGRP` parameter in `CRTSRVPGM` command help.                                                                                                                                                                |*CALLER    |
    |`module`     |Module                         |Refer to the `MODULE` parameter in `CRTSRVPGM` command help.                                                                                                                                                                |           |
    |`pOption`    |Pre-compiler OPTIONS           |Refer to the `OPTION` parameter in `CRTSQLCBLI` command help.                                                                                                                                                               |*SYSVAL    |
    ...
    |`incDir`     |Include Directory              |Specifies one or more directories to add to the search path used by the compiler to find copy files. The compiler will search the directories specified here if the copy files in the source program can not be resolved.   |*NONE      |
    |`tgtCcsid`   |Target CCSID                   |Specifies the CCSID that the compiler uses to read the source files.                                                                                                                                                        |*JOB       |
    |`proxyCmd`   |Proxy Command                  |Specifies a command to substitute the `RUCRTCBL` command.                                                                                                                                                                   |           |
  ...
  <TabItem label="CODECOV" >
    ...
    |`proxyCmd`   |Proxy Command   |Specifies a command to substitute the `CODECOV` command.                                    |        |
  ...
  <TabItem label="RUCALLTST" >
    ...
    |`proxyCmd`   |Proxy Command     |Specifies a command to substitute the `RUCALLTST` command.                                                                                                                                                                    |         |
...
## Vendor Integration
The `proxyCmd` and `wrapperCmd` parameters in the `RUCRTRPG`/`RUCRTCBL`/`RUCALLTST`/`CODECOV` specifications are not actual parameters of the RPGUnit or CODECOV commands, but rather are provided by the extension to support vendor tool integration.
