...
<Aside type="tip">
  By default the **Refactor Preview** view displays the edits in the `Group Changes By File` mode. Typically, switching to `Group Changes By Type` (as shown below) is more helpful in understanding the purpose of each edit. You can change to this mode by clicking the button in the top right of the panel.
  ![Group Changes By Type](Testing_GroupChangesByType.png)
</Aside>
